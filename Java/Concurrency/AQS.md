...
1. ``state``
    ...
    首先，``AQS``类中维护了一个``state``字段，并且该字段由``volatile``修饰，它就是所谓的同步状态。由于是由``volatile``修饰，所以任何线程对其进行修改对于其他线程都是立即可见的。
    ...
    **说明**：
    ...
    这些方法都是``final``修饰的，说明子类无法重写他们。


    独占模式 ``Exclusive Mode``与共享模式``Shared Mode``。在独占模式下，一个线程在执行某些操作的时候，其他线程不可以执行这些操作。比如持有独占锁的操作，一个线程持有了独占锁，那么其他线程不能持有锁。而共享模式允许多个线程同时进行某种操作。

    在独占模式下，我们可以把``state``字段的初始值设置为0，每当某个线程要进行某项操作**之前**，都需要判断``state``的值是不是0，如果不是0的话意味着别的线程已经进入该操作，那么本线程需要等待，如果是0的话就把``state``字段的值设置为1。自己进入该操作。这个先判断为0在设置为1的过程可以通过``CAS``操作来保证原子性。

    在共享模式下，比如我们把某项操作设置为最多允许10个线程同时进行。那么我们就可以把``state``的初始值设置为10，一个线程尝试获取锁成功，那么减1。

    ---

2. 同步队列

    1. 同步队列是一个双向队列（即每一个节点都有一个``prev``和``next``，分别指向前一个节点和后一个节点），且满足``FIFO``。``tail``和``head``分别指向队首和队尾
    ...
    2. 这个队列的节点类被定义成了一个静态的内部类，主要字段如下：
        ...
        |``Node.SIGNAL``|-1|表示当前节点的后续节点被阻塞，当前节点在释放或者取消时，要将后续节点解除阻塞|
    ...
    3. 独占状态下同步状态的获取与释放
        ...
        + ``next``：该变量类型为``Node``类型，指向当前节点的后一个节点（后继节点）
        + 该变量类型为``Thread``，表示该节点所代表的线程
        + ``nextWaiter``：该变量类型为``Node``类型
    ...
    4. 实现插入等待队列
        ...
        源码如下：
            ...
            final boolean acquireQueued(final Node node, int arg) {
                ...
                try {
                    ...
                    for (; ; ) {
                        ...
                        if (shouldParkAfterFailedAcquire(p, node) &&parkAndCheckInterrupt())  // 获取不到锁，那么直接阻塞，阻塞结束仍在在循环中获取锁
                            interrupted = true;
                    }
                } finally {
                    if (failed)
            ...
            流程图补充说明：紫色箭头所在的流程是一个``for(;;)``循环。而跳出该循环的唯一条件是``prev``节点是``head``节点，并且当前线程获取锁成功。
        ...
        之后``if (shouldParkAfterFailedAcquire(p, node) &&parkAndCheckInterrupt())``条件不成立，再次进行``for(;;)``循环，第二次执行到该方法那么该函数返回``true``，从而执行``parkAndCheckInterrupt()``方法将线程阻塞。
...
3. 公平锁与非公平锁
不管是在公平锁还是非公平锁的模式下，``ReentrantLock``对于同步队列中的线程都能**保证排在队列前面的线程一定比排在队列后面的线程优先获取到锁**。但是非公平锁不保证**队列中的第一个线程就一定比先来的线程（此时还未加入到同步队列）优先获取到锁**。因为队列中的第一个线程尝试获取锁时，可能刚好来了一个线程获取锁，而这个线程还没有加到同步队列，从而获取到锁。
对于``ReentrantLock``，提供给外部的加锁的源码如下：

    ```java
    public void lock() {
        sync.lock();  // 根据sync具体指向的实例对象来进行执行不同的lock()方法
    }
    ```

    根据``ReentrantLock``实例化对象时创建的是公平锁还是非公平锁，``sync``指向的的实例对象不同。如果是公平锁，那么``sync``指向``FairSync``。如果是非公平锁，那么``sync``指向``NonfairSync``。但``FairSync``和``NonfairSync``均继承自``Sync``。而``Sync``继承``AQS``。
    ...
    /*
    不论是公平锁还是非公平锁，释放锁的代码都是一样的。
    */
    protected final boolean tryRelease(int releases) {
            int c = getState() - releases;  // 将同步状态自减1
    ...
    **非公平锁**
    ...
    final void lock() {
        /*
        非公平锁调动lock()会直接尝试获取锁，获取到了，那么就直接执行获取锁后的逻辑。如果失败，那么执行acquire()尝试获取锁。
        */
    ...
    protected final boolean tryAcquire(int acquires) {
            /*
            而非公平锁tryAcquire()本质上是调用NonfairSync类中的方法nonfairTryAcquire()，
            NonfairSync继承自Sync
             */
    ...
    final boolean nonfairTryAcquire(int acquires) {
        ...
        if (c == 0) {
            ...
            前者仅仅多了一个hasQueuedPredecessors()方法判断当前同步队列是否有等待线程
            */
...
4. ``await/signal``等待唤醒机制

    为了实现等待唤醒机制，``AQS``类中有一个内部类，``ConditionObject``，而这个类实现了``Condition``接口，这个接口提供了以下几个方法
    ...
    public abstract class AbstractQueuedSynchronizer {
        ...
        public class ConditionObject implements Condition, java.io.Serializable {
            private transient Node firstWaiter;  // 内部有两个Node类型的引用，分别指向等待队列的头节点和尾节点。
    ...
    1. ``await()``方法
        ...
        + 执行流程：
            ...
            + 此时由于``main``线程获取到了锁处于运行的状态，但是运行时发现某个条件不满足，所以选择执行以下代码进入到``condition1``等待队列：
                ...
                3. 因为此时``main``线程还持有锁，所以在释放锁之后唤醒同步队列中等待时间最长的线程。该线程会尝试获取锁。如果获取锁成功，那么此时等待队列如下：
    ...
    3. 总结：
        ...
        + 内置锁即``synchronized``只能对应于一个等待队列，而``ReentrantLock``可以产生若干个等待队列，我们可以根据线程的不同等待条件来把线程放到不同的等待队列中。

        + 每一个显示锁对象可以产生若干个``Condition``对昂，每一个``Condition``对象都会对应一个等待队列。即一个显示锁可以对应多个等待队列。
...
5. 实现自己的同步器
    ...
    */
    ...
    public class MutexLock {
        ...
        3.不管怎样，加锁方法调用的是acquire()方法（该方法提供了阻塞的操作）
        */
        ...
        public void unlock() {
            sync.release(1);  // 该方法提供了将阻塞的线程唤醒的操作
        }
    }
    ...
    */
    public class Mutex implements Lock {
        ...
        */
        ...
        public Condition newCondition() {
            return sync.newCondition();  // 对外提供的方法，用来创建Condition对象
        }
